 ...
 * 
 */
public class DomainPackagePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public DomainPackagePropertiesEditionComponent(PropertiesEditingContext editingContext, EObject domainPackage, String editing_mode) {
		super(editingContext, domainPackage, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = UmaViewsRepository.class;
		partKey = UmaViewsRepository.DomainPackage.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final DomainPackage domainPackage = (DomainPackage)elt;
			final DomainPackagePropertiesEditionPart basePart = (DomainPackagePropertiesEditionPart)editingPart;
			// init values
			if (domainPackage.getName() != null && isAccessible(UmaViewsRepository.DomainPackage.Properties.name))
				basePart.setName(EEFConverterUtil.convertToString(EcorePackage.eINSTANCE.getEString(), domainPackage.getName()));
			
			if (isAccessible(UmaViewsRepository.DomainPackage.Properties.ownedMethodContentMember)) {
				ownedMethodContentMemberSettings = new ReferencesTableSettings(domainPackage, SpemPackage.eINSTANCE.getMethodContentPackage_OwnedMethodContentMember());
				basePart.initOwnedMethodContentMember(ownedMethodContentMemberSettings);
			}
			if (isAccessible(UmaViewsRepository.DomainPackage.Properties.reusedPackage)) {
				reusedPackageSettings = new ReferencesTableSettings(domainPackage, SpemPackage.eINSTANCE.getMethodContentPackage_ReusedPackage());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == UmaViewsRepository.DomainPackage.Properties.name) {
			return SpemPackage.eINSTANCE.getMethodContentPackageableElement_Name();
		}
		if (editorKey == UmaViewsRepository.DomainPackage.Properties.ownedMethodContentMember) {
			return SpemPackage.eINSTANCE.getMethodContentPackage_OwnedMethodContentMember();
		}
		if (editorKey == UmaViewsRepository.DomainPackage.Properties.reusedPackage) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		DomainPackage domainPackage = (DomainPackage)semanticObject;
		if (UmaViewsRepository.DomainPackage.Properties.name == event.getAffectedEditor()) {
			domainPackage.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.eINSTANCE.getEString(), (String)event.getNewValue()));
		}
		if (UmaViewsRepository.DomainPackage.Properties.ownedMethodContentMember == event.getAffectedEditor()) {
		...
		if (UmaViewsRepository.DomainPackage.Properties.reusedPackage == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {	
			DomainPackagePropertiesEditionPart basePart = (DomainPackagePropertiesEditionPart)editingPart;
			if (SpemPackage.eINSTANCE.getMethodContentPackageableElement_Name().equals(msg.getFeature()) && basePart != null && isAccessible(UmaViewsRepository.DomainPackage.Properties.name)) {
			...
			if (SpemPackage.eINSTANCE.getMethodContentPackage_OwnedMethodContentMember().equals(msg.getFeature()) && isAccessible(UmaViewsRepository.DomainPackage.Properties.ownedMethodContentMember))
				basePart.updateOwnedMethodContentMember();
			if (SpemPackage.eINSTANCE.getMethodContentPackage_ReusedPackage().equals(msg.getFeature())  && isAccessible(UmaViewsRepository.DomainPackage.Properties.reusedPackage))
