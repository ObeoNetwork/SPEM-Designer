 ...
 * 
 */
public class SpemSampleWizardPage extends WizardPage {
	 ...
	 * @param pageName
	 */
	protected SpemSampleWizardPage(String pageName) {
		super(pageName);
	}
	 ...
	 * @param titleImage
	 */
	protected SpemSampleWizardPage(String pageName, String title,
	...
	public void createControl(Composite parent) {
		...
		table.addListener(SWT.SELECTED, new Listener() {
			public void handleEvent(Event event) {
				SpemSampleWizardPage.this.handleEvent();
			}
		});
		...
		for (SpemSample spemSample : ((ProjectUnzipperNewWizard) this
				.getWizard()).getSpemSamples()) {
			tableItem = new TableItem(table, SWT.NONE);
			tableItem.setText(spemSample.getTitle());
		}
	...
	private void handleEvent() {
		if (this.getSelection() > -1) {
			browser.setText(((ProjectUnzipperNewWizard) this.getWizard())
					.getSpemSamples().get(table.getSelectionIndex())
					.getDescription());
			ImageDescriptor imageDescriptor = ((ProjectUnzipperNewWizard) this
					.getWizard()).getSpemSamples().get(
