...
public class Utils {
	...
	public static List<IGendoc2Runner> getAllRunners() {
		if (runners == null) {
			...
			IConfigurationElement[] elements = Platform.getExtensionRegistry()
					.getConfigurationElementsFor(Activator.PLUGIN_ID, "runner");
			for (IConfigurationElement e : elements) {
				try {
					IGendoc2Runner runner = (IGendoc2Runner) e
	...
	/**
	 * @return the list of all extensions of the template we need in order to
	...
	/**
	 * @param fileName
	 *            name of the model selected in order to generate its
	 *            documentation
	...
	public static List<IGendoc2Runner> getRunners(String fileName) {
		List<IGendoc2Runner> runners = getAllRunners();
		List<IGendoc2Runner> result = new ArrayList<IGendoc2Runner>(
				runners.size());
